...
var DEFAULT_RATE_LIMIT_PER_HOUR = 3;
var RATE_LIMIT_TTL_SECONDS = 3600;
var corsHeaders = {
...
async function handleRequest(request, env = {}) {
  ...
  try {
    ...
    const trackingUrl = discord.url || env.PUBLIC_TRACKING_URL || "https://discord.com";
...
function buildSubmission(payload, now = /* @__PURE__ */ new Date()) {
  ...
  if (kind === "song_request") {
    ...
    return {
      ...
      fields: [
        ...
        ["Lyrics or source link", required(payload, "lyricsOrSource", 6e3)],
        ["Additional notes", optional(payload, "notes", 2e3)]
      ]
    };
  }
  if (kind === "problem_report") {
    ...
    return {
      ...
      fields: [
        ["Summary", summary],
        ["What happened?", required(payload, "description", 4e3)],
  ...
  if (kind === "song_correction") {
    ...
    return {
      ...
      fields: [
        ...
        ["What should be corrected?", required(payload, "correction", 4e3)],
        [
          "Suggested correction or source",
          optional(payload, "suggestedCorrectionOrSource", 4e3)
        ]
      ]
    };
  }
...
__name(buildSubmission, "buildSubmission");
...
async function postDiscordSubmission(submission, env) {
  ...
  return {
    id: messageId,
    url: guildId && channelId && messageId ? `https://discord.com/channels/${guildId}/${channelId}/${messageId}` : null
  };
}
__name(postDiscordSubmission, "postDiscordSubmission");
    ...
    embeds: [
      {
        ...
        fields: submission.fields.filter(([, value]) => value).map(([name, value]) => ({
          name,
...
__name(discordPayload, "discordPayload");
...
__name(truncateForDiscord, "truncateForDiscord");
...
__name(submissionId, "submissionId");
...
__name(messageNumber, "messageNumber");
