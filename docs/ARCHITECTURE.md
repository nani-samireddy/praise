...
## 20. Support and moderation boundary

V1 uses a low-maintenance support intake rather than requiring users to own a
Discord or GitHub account. Settings exposes song-request and general-report
forms, while a server song exposes a correction form from its detail menu. The
app submits the validated form to a small stateless HTTPS service and receives a
tracking/reference number and URL. The user can copy that URL or open it later
through `url_launcher`.

The primary support service runs from `support_worker/` as a Cloudflare Worker.
It validates accepted fields, limits request size, optionally rate-limits by IP
with Cloudflare KV, posts a structured embed into a Discord moderation channel,
and returns the Discord message link when Discord provides one. It stores no
database or local files and never logs submitted bodies.

The legacy Render/GitHub issue relay in `support_api/` remains as a fallback,
but production builds should use the Worker endpoint once deployed.

Discord webhook credentials are never included in the APK. Before submission,
Praise states that a support request will be created and tells the user not to
include private information. Song corrections contain public catalogue
identifiers and titles; general device details are entered voluntarily. The
public endpoint remains an abuse surface even with lightweight rate limiting, so
Discord interaction review, CAPTCHA/Turnstile, or stronger attestation should be
added if usage grows.
